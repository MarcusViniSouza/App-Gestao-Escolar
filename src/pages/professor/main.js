...
export default class Main extends Component {
  ...
  getUsuarioStorage = async () => {
    const usuario = await getRegistroStorage('dadosusuario');

    if (usuario) {
      const { LOGIN, SENHA } = usuario;
      await this.handleUsuario(LOGIN);
      await this.handleSenha(SENHA);
    }
  }
  ...
  setDadosUsuario = async (usuario) => {
    try {
      await setRegistroStorage('dadosusuario', usuario);
    } catch (e) {
  ...
  salvarAcesso = async (codigo) => {
    try {
      var parametros = { ID_USER: codigo, TIPO_ACESSO: 'IN' };
  ...
  handleSignInPress = async () => {
    ...
    if (this.state.nomeMunicipio === '') {
      ...
      this.mensagem('Warning', 'Preencha usuário e senha para continuar!');
    } else {
      ...
      try {
        ...
        const response = await getApi(`logar/${user}/${MD5(pass)}`);

        const { ID, NOME, LOGIN, CSI_CODPRO } = response.result[0];

        if (LOGIN !== '') {

          const dados = { ID: ID, NOME: NOME, LOGIN: LOGIN, ID_PROF: CSI_CODPRO, SENHA: this.state.senha }
          await this.setDadosUsuario(dados);

          await this.salvarAcesso(ID);

          const { navigate } = this.props.navigation;
          navigate('Escola', { codigo: ID });

        } else {
          this.mensagem('Warning', 'Houve um problema com o login, verifique suas credenciais!');
        }
      }
      catch (e) {
  ...
  enviarSenha = async () => {
    try {
      if (this.state.nomeMunicipio === '') {
        ...
        var parametros = {user: this.state.usuario, tipo: 'P'};
